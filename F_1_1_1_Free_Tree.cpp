...
vector<ll> vec(N);
vector<ll> parnt(N);  
vector<ll> sum(N), wt(N);
vector<map<ll, ll>> colsum(N);
ll ans;
void dfs(int vertex, int par = -1)
{
    ...
    for (auto child : g[vertex])
    {
        ...
        parnt[child.first] = vertex;
        sum[vertex] += child.second;
        colsum[vertex][vec[child.first]] += child.second;
        wt[child.first] = child.second;
...
int main()
{
    ...
    while (t--)
    {
        ...
        while (q--)
        {
            ...
            if (v)
            {
                if (vec[v] != vec[parnt[v]])
                {
                    ans -= wt[v];
                }
                colsum[parnt[v]][vec[v]] -= wt[v];
            }
            vec[v] = x;
            ans += sum[v];
            ans -= colsum[v][vec[v]];
