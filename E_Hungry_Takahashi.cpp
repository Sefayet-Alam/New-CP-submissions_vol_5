...
bool can_survive(ll x) {
    vector<vector<ll>> dp(H, vector<ll>(W, -1));
    

    if (x + A[0][0] < P[0]) return false;
    dp[0][0] = x + A[0][0] - P[0];
    ...
    for (int i = 0; i < H; ++i) {
        for (int j = 0; j < W; ++j) {
            ...
            if (i + 1 < H) {
                ll coins = dp[i][j] + A[i + 1][j];
                int next_day = day + 1;
                if (coins >= P[next_day]) {
                    dp[i + 1][j] = max(dp[i + 1][j], coins - P[next_day]);
                }
            }

            if (j + 1 < W) {
                ll coins = dp[i][j] + A[i][j + 1];
                int next_day = day + 1;
                if (coins >= P[next_day]) {
                    dp[i][j + 1] = max(dp[i][j + 1], coins - P[next_day]);
                }
            }
        }
    }
