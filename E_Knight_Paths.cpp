...
int main()
{
        ...
        for(ll j=0;j<8;j++){
            for(auto it:directions){
               ...
               if(newi>=0 && newi<8 && newj>=0 && newj<8){
                mat.a[i*8+j][newi*8+newj]=1; // means we can move to newi,newj from i,j...think of it as a graph
               }
            }
        }
    }
    ...
    for(ll i=0;i<=64;i++){
        mat.a[i][64]=1;//from each point we can move to the sum..64th cells mentions sum
    }
    while(t--){
      ll n;
      cin>>n;
