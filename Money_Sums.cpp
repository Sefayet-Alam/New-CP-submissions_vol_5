...
int main()
{
    ...
    for (ll i = 1; i <= n; i++)
    {
        ll now=(i%2);
        ll prev=(now^1);
        for (ll current_sum = 0; current_sum <= MAX_SUM; current_sum++)
        {
            dp[now][current_sum] = dp[prev][current_sum];
            ll prev_sum = current_sum - coins_values[i-1];
            if (prev_sum >= 0 && dp[prev][prev_sum])
            {
                dp[now][current_sum] = true;
                possible.insert(current_sum);
            }
        }
    }
    cout<<possible.size()<<nn;
    cout<<possible<<nn;
}
