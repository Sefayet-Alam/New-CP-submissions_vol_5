...
const ll maxnodes = 5005; //be careful!..print function uses memory!
const ll inf=M;
...
void printMaxFlowPaths(bool output_paths = true) {
    ll total_flow = maxFlow(src, dest);
    ...
    // OPTIMIZED: Use adjacency list instead of matrix
    vector<vector<pair<ll, ll>>> flow_graph(nodes); // (neighbor, flow)
    for (ll u = 0; u < nodes; u++) {
        for (Edge &e : g[u]) {
            if (e.f > 0) {
                flow_graph[u].push_back({e.to, e.f});
            }
        }
    }
    
    // Extract paths - O(F × (V + E))
    ll path_count = 0;
    while (path_count < total_flow) {  // At most F paths
        ...
        for (ll v = dest; v != src; v = parent[v]) {
            ll u = parent[v];
            // Find flow value - O(degree(u))
            for (auto &[neighbor, flow] : flow_graph[u]) {
                if (neighbor == v) {
