...
vector<ll>vec(N);
ll dp[N][2][2];
ll func(ll i,bool st,bool en){
    if(i==n){
        return 0;
    }
    if(dp[i][st][en]!=-1) return dp[i][st][en];
    ll ret=0;
    if(!st){
        ret=max(ret,abs(vec[i])+func(i+1,st,en));
        ret=max(ret,vec[i]*2+func(i+1,1,0));
    }
    else if(st && en==0){
        ret=max(ret,vec[i]*2+func(i+1,1,0));
        ret=max(ret,abs(vec[i])+func(i+1,1,1));
    }
    else{
        ret=max(ret,abs(vec[i])+func(i+1,1,1));
    }
    return dp[i][st][en]=ret;
