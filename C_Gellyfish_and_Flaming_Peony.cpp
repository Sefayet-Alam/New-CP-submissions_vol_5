...
ll func(ll i, ll gcdd)
{
    ...
    if (dp[i][gcdd] != -1)
        return dp[i][gcdd];
    ll ret = M;
    ll curg = GCD(gcdd, vec[i]);
    ret = min(ret, 1 + func(i + 1, curg));
    ret = min(ret, func(i + 1, gcdd));
    return dp[i][gcdd] = ret;
}
...
int main()
{
    ...
    while (t--)
    {
        ...
        cin >> vec;
        sort(all(vec));
        ll maxno = *max_element(all(vec));
        for (ll i = 0; i <= n; i++)
        {
            for (ll j = 0; j <= maxno + 5; j++)
                dp[i][j] = -1;
        }
        g = 0;
        for (auto it : vec)
