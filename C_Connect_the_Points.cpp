...
int dist(vector<pair<int,int> > v, pair<int,int> p){
	return dist(v[0] , p) + dist(v[1] , p) + dist(v[2] , p);
}
...
void solve()
{
	...
	for(auto &x : xs){
		for(auto &y : ys){
			int cur = dist(v, {x, y});
			if(cur < mn) ans = {x, y} , mn = cur;
		}
	}
	cout << 6 << '\n';
	print(v[0] ,{v[0].first, ans.second});
	print({v[0].first, ans.second},ans);
	print(v[1] ,{v[1].first, ans.second});
	print({v[1].first, ans.second},ans);
	print(v[2] ,{v[2].first, ans.second});
	print({v[2].first, ans.second},ans);
