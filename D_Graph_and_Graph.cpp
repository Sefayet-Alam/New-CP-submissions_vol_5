...
int main()
{
    ...
    while (t--)
    {
        ...
        while (pq.size())
        {
            ll v1 = pq.top().second.first;
            ll v2 = pq.top().second.second;
            ll v_dist = pq.top().first;
            pq.pop();
            // deb2(v1,v2);
            if(dist[v1][v2]<v_dist) continue;
            for (auto &child : g[v1])
            {
                for(auto &child2:g2[v2]){
                    ll wt=abs(child-child2);
                    if(dist[v1][v2]+wt<dist[child][child2]){
                        dist[child][child2]=dist[v1][v2]+wt;
                        pq.push({dist[child][child2],{child,child2}});
                    }
                }
            }
        }
