...
ll func(ll i, ll tek)
{
    ...
    if (freq[i])
    {
        ll a = func(i + 1, tek + freq[i]);
        ll b = func(i + 1, tek);
        return dp[i][tek] = (a + b) % M;
    }
    else
    {
        ret = func(i + 1, tek);
...
int main()
{
    ...
    while (t--)
    {
        ...
        // mem(dp, -1);
        // deb(n);
        consst = (factorialMod(n / 2, M) * factorialMod(n - n / 2, M)) % M;
        ll down = 1;
        for (ll i = 0; i < 26; i++)
        {
            if (freq[i])
            {
                ll cur = inverseMod(factorialMod(freq[i], M), M);
                down = (down * cur) % M;
            }
        }
        consst = (consst * down) % M;
