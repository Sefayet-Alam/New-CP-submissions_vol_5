...
/*
 dp[𝑢] as the number of back-edges passing over the edge between 𝑢 and its parent. Then,
dp[𝑢]=(# of back-edges going up from 𝑢)−(# of back-edges going down from 𝑢)+∑dp[𝑣] -> 𝑣 is a child of 𝑢.
    ...
    for (int nxt : adj[vertex])
    {
        if (lvl[nxt] != 0)
        {
            if (nxt != parent)
            {
                ...
                {
                    dp[vertex]--; //this edge is to down edge
                    ans.push_back(make_pair(nxt, vertex)); 
                }
                else if (lvl[nxt] < lvl[vertex])
                { 
                    dp[vertex]++; //this edge is to up edge
                }
            }
        }
        else
