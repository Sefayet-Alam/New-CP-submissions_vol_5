...
void polar_sort(vector<pair<PT, char>> &v)
{ // sort points in counterclockwise
    sort(v.begin(), v.end(), [](pair<PT, char> aa, pair<PT, char> bb)
        { 
            auto a=aa.first;
            auto b=bb.first;
    ...
    for (ll i = 0; i < n; i++)
    {
        for (ll j = 0; j < n; j++)
        {
            for (ll k = 0; k < n; k++)
            {
                if (i == j || j == k || k == i)
                    continue;
                vector<pair<PT, char>> tmp = {vec[i], vec[j], vec[k]};
                string p = "";
                p += tmp[0].second;
                p += tmp[1].second;
                p += tmp[2].second;

                if (p[0] == p[2] || p[0]==p[1] || p[1]==p[2])
                {
                    if (area_of_triangle(tmp[0].first, tmp[1].first, tmp[2].first) != 0.00000)
                    {
                        // for (auto it : tmp)
                        // {
                        //     // cout << it.first.x << " " << it.first.y << nn;
                        // }
                        ans = 3;
                        break;
                    }
