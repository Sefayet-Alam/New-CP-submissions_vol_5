...
ll func(ll i, bool f)
{
    ...
    if (!f)
    {
        if (mpp.find(vec[i] + 1) != mpp.end())
        {
            auto nxt = upper_bound(all(mpp[vec[i] + 1]), i);
            if (nxt != mpp[vec[i] + 1].end())
                ret = max(ret, 1 + func(*nxt, 1));
        }
