...
string retstring(string s, pll ans)
{
    ll n = s.size();
    string ret = "";
    for (ll i = 0; i <= ans.first; i++)
        ret += s[i];
    for (ll i = ans.second; i < n; i++)
        ret += s[i];
    return ret;
}

string calc(string s, ll n)
{
    // deb(s);
    PolyHash hs(s);
    pll ans = {0, n};
    ...
    for (ll i = 0; i < n; i++)
    {
        if (hs.get_hash(0, i) == hs.rev_hash(0, i))
        {
            ll nowsz = 2 * (i + 1);
            if (nowsz > n)
                continue;
            ...
            while (l <= r)
            {
                ...
                else if (hs.get_hash(sti, mid) != hs.rev_hash(sti, mid))
                {
                    r = mid - 1;
                }
                else
...
int main()
{
    ...
    while (t--)
    {
        ...
        // string p=s;
        // reverse(all(p));
        string ans1 = calc(s, n);
        string p = s;
        reverse(all(p));
        string ans2 = calc(p, n);
        // deb2(ans1, ans2);
