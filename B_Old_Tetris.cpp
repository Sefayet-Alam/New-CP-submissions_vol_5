...
int main()
{
    ...
    BigInt ten("10"), fifty("50"), twenty("20"), tw("2"), on("1"), thrty("30");
    BigInt lvl2("0");
    while (l <= r)
    {
        BigInt mid = l + (r - l) / tw;
        ...
        BigInt evlvl = ((mid) / tw);
        BigInt odlvl = (mid - evlvl);
        BigInt now = thrty * odlvl * odlvl + twenty * (evlvl * (evlvl + on));
    ...
    BigInt evlvl = ((lvl2) / tw);
    BigInt odlvl = (lvl2 - evlvl);
    BigInt now = thrty * odlvl * odlvl + twenty * (evlvl * (evlvl + on));
    ...
    if (odlvl >= on)
    {
        if (odlvl > evlvl)
        {
            BigInt last = (tw * odlvl - on) * ten;

            if (now >= last + x && lvl2 >= on)
            {
                lvl2 = lvl2 - on;
                while (now >= last + x)
                {
