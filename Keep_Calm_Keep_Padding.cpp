...
vector<ll> x(N), rem(N), sum(N);
vector<pll> ans;

void sumdfs(ll u, ll p)
{
    sum[u] = rem[u];
    for (auto v : g[u])
    {
...
void dfs(ll u, ll p, ll up)
{
    ...
    for (auto v : g[u])
    {
        if (v == p)
            continue;
        children.push_back({sum[v], v});
    }
    sort(all(children), greater<pll>());

    if (rem[u] < 0)
    {
        if (up + rem[u] >= 0)
        {
            ans.push_back({u, rem[u]});
            up += rem[u];
            rem[u] = 0;
        }
        else
    ...
    if (children.size())
    {
        ...
        for (ll j=0;j<children.size();j++)
        {
            ...
            dfs(it.second, u, cur);
            cur += sum[it.second];
            if(j<children.size()-1) ans.push_back({u,0});
        }
        if (rem[u] < 0)
        {
            if (cur + rem[u] >= 0)
