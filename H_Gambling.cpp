...
int main()
{
    ...
    while (t--)
    {
        ...
        for (ll i = 1; i <= n; i++)
            cin >> vec[i];
        map<ll, ll> last, maxsc, freq;
        map<ll, ll> l, r;
        for (ll i = 1; i <= n; i++)
        {
            // deb(vec[i]);
            freq[vec[i]]++;
            ll othrs = ((i)-last[vec[i]] + 1) - freq[vec[i]];
            ll curscr = freq[vec[i]] - othrs;
            if(curscr>maxsc[vec[i]]){
                maxsc[vec[i]]=curscr;
                l[vec[i]]=last[vec[i]];
                r[vec[i]]=i;
            }
