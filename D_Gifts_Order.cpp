...
/// calc calculates maximum subsegment sum from l to r-1
struct item
{
    ll val, minm, maxm;
};
struct segment_tree
{
    ...
    item neutral_ele = {0LL, M, -M};
    item merge(item a, item b)
    {
        item ret;
        ret.maxm = max(a.maxm, b.maxm);
        ret.minm = min(a.minm, b.minm);
        ret.val = max({a.val, b.val, b.maxm - a.minm});
        return ret;
    }
...
int main()
{
    ...
    while (t--)
    {
        ...
        vector<ll> rev = vec;
        reverse(all(rev));
        segment_tree sg, sgrev;
        sg.init(n), sgrev.init(n);
        sg.build(vec);
        sgrev.build(rev);
        ll a = sg.calc(0, n).val;
        ll b = sgrev.calc(0, n).val;
        cout << max(a, b) << nn;
        while (q--)
        {
            ll x, y;
