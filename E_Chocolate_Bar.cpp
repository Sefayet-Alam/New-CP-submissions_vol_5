    ...
    for(ll i=0;i<N;i++){
        for(ll j=0;j<N;j++){
            for(ll l=0;l<K;l++){
                ...
                dp[i][j][l]=M*M;
                for(ll i1=1;i1<i;i1++){
                    //each part should hav one row
                    
                    for(ll l1=0;l1<=l;l1++){
                        // l1 can be zero->nothing eaten
                        ll i2=i-i1;
                        ll l2=l-l1;
                        dp[i][j][l]=min(dp[i][j][l],j*j+dp[i1][j][l1]+dp[i2][j][l2]);
                    }
                }
                for(ll j1=1;j1<j;j1++){
                    for(ll l1=0;l1<=l;l1++){
                        ll j2=j-j1;
                        ll l2=l-l1;
                        dp[i][j][l]=min(dp[i][j][l],i*i+dp[i][j1][l1]+dp[i][j2][l2]);
                    }
                }
            }
        }
