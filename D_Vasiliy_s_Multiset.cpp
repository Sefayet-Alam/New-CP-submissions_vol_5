...
struct Trie
{
    ...
    const int B = 32; // Since numbers are up to 1e9, 31 bits are sufficient
    struct node
    {
        node *nxt[2];
        int sz;
    ...
    ll count(ll val, ll k)
    {
        ...
        // Add 1 if we found an exact match (reached end of path)
    ...
    int get_min(int x)
    {
        ...
        for (int i = B - 1; i >= 0; i--)
        {
            ...
            {
                cur = cur->nxt[k];
                ans = (ans << 1); // Same bit -> XOR result 0
            }
            else
            {
                cur = cur->nxt[!k];
                ans = (ans << 1) | 1; // Different bit -> XOR result 1
            }
        }
        return ans;
    }
...
int main()
{
    ...
    trie.insert(0); // Initialize with 0 to match jury's answer
    freq[0]++;
    while (q--)
    {
        char op;
