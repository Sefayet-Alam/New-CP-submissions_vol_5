...
int main()
{
    ...
    ll tot1 = accumulate(all(a), 0LL);
    ll tot2 = accumulate(all(b), 0LL);
    if (tot1 != tot2)
    {
        cout << -1 << nn;
        return 0;
    ...
    for (ll i = 0; i < n; i++)
    {
        addEdge(src, i + 1, a[i], 0);
        addEdge(i + 1 + n, dest, b[i], 0);
        for (ll j = 0; j < n; j++)
        {
            ll x;
            cin >> x;
            grid[i][j] = x;
            addEdge(i + 1, j + n + 1, 1, -x);
        }
    }

    auto ans = minCostMaxFlow(src, dest);
    if (ans.first != tot1)
    {
        cout << -1 << nn;
        return 0;
    }
    ll maxCoins = -ans.second;
    cout << maxCoins << "\n";
