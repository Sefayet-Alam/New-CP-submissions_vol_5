...
int main()
{
    ...
    while (t--)
    {
        ...
        cin >> n>>m;
        // vector<ll>b;
        map<ll,ll>ciph,deciph;
        vector<ll> vec(n);
        ll cc=0;
        for (ll i = 0; i < n; i++){
            cin >> vec[i];
            if(ciph.find(vec[i])==ciph.end()){
                ciph[vec[i]]=cc;
                deciph[cc]=vec[i];
                cc++;
            }
        ...
        while (m--)
        {
            ...
            set<ll>ans;
            for(ll i=1;i<=50;i++){
                ll now=gen_random(l,r);
                ll k=vec[now];
                
                ll cont=upper_bound(all(pos[k]),r)-lower_bound(all(pos[k]),l);
                if(cont>lim) ans.insert(deciph[k]);
            }
