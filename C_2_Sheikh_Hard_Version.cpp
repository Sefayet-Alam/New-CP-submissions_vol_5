...
void solve(){
   ...
   while(q--){
    ll l,r; cin >> l >> r;
    if((sum[r] - sum[l - 1]) - (x[r] ^ x[l - 1]) == 0){
        cout << l << ' ' << l << '\n';
        continue;
    }
    ll g = (sum[r] - sum[l - 1]) - (x[r] ^ x[l - 1]);
    pll best = {l, r}; 
    for(ll i = l, c1 = 0; i <= r && c1 <= 32; c1++){
        for(ll j = r, c2 = 0; j >= i && c2 <= 32; c2++){
           // cout << "! "  << i << ' ' << j << '\n';
            if((sum[j] - sum[i - 1]) - (x[j] ^ x[i - 1]) == g) best = pick(best, make_pair(i, j));
            else{
                break;
            }
            j = L[j];
