...
void forward1(ll dp[])
{ // adding element to all its super set
    // dp[i]->no of elements that are in i's subset
...
{ // undo of forward function
    // add a[i] to a[j] if j&i = i
    for (ll bit = 0; bit <= MLOG; bit++)
    {
...
void forward2(ll dp[])
{ // add elements to its subsets
    // dp[i]->no of elements that are in i's superset
    for (ll bit = 0; bit <= MLOG; bit++)
    {
        for (ll i = MAXN - 1; i >= 0; i--)
        {
...
int main()
{
    ...
    forward2(forw2ans);
    for(ll i=0;i<n;i++){
        ll ans1=forw1ans[vec[i]];
        ll ans2=forw2ans[vec[i]];
        ll ans3=n-forw1ans[(MAXN-1)^vec[i]];
        cout<<ans1<<" "<<ans2<<" "<<ans3<<nn;
    }
    return 0;
}
