...
void bfs(const vector<ll>& srcs, ll n) {
    for (ll i = 1; i <= n; i++) {
        level[i] = -1;
        par[i] = 0;
    }
...
int main() {
    ...
    bfs({1}, n);
    ll a = 1;
    for (ll i = 1; i <= n; i++) if (level[i] > level[a]) a = i;
    bfs({a}, n);
    ll b = 1;
    for (ll i = 1; i <= n; i++) if (level[i] > level[b]) b = i;

    ll diameter_len = level[b];
