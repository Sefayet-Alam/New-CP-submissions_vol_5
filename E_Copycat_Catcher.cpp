...
int main()
{
    ...
    while (q--)
    {
        ...
        PolyHash hs2(quer);
        ll sz = quer.size();
        ll val = hs2.get_hash(0, sz - 1);
        ll l = 0;
        bool wow = 0;
        for (ll i = 0; i + sz - 1 < main.size(); i++)
        {
            bool ok = 0;
            ll mval = Mhs.get_hash(i, i + sz - 1);
            if (val == mval)
            {
                ok = 1;
                // deb(i);
                // deb2(val, mval);
                vector<ll> mpp(300, -1), mpp2(300, -1);
                for (auto it : singls)
                {
                    char now = unch2[it];
                    char oth = unch[i + it];
                    if (mpp[oth - 'A'] == -1 && mpp2[now - 'A'] == -1)
                    {
                        mpp[oth - 'A'] = now - 'A';
                        mpp2[now - 'A'] = oth - 'A';
                    }
                    else if (mpp[oth - 'A'] != now - 'A' || mpp2[now - 'A'] != oth - 'A')
                    {
                        ok = 0;
                        break;
                    }
