...
int main()
{
    ...
    while (t--)
    {
        ...
        while (q--)
        {
            ll k;
            cin >> k;
            unordered_map<ll, vl> cntr;
            for (ll i = 0; i < k; i++)
            {
                ll x;
                cin >> x;
                cntr[par[x]].push_back(x);
            }
            for (auto it : cntr)
            {
                ll prevpar = it.first;
                ll tot = gsz[prevpar];
                ll sznow = it.second.size();
                if (sznow < tot)
                {
                    szs[tot]--;
                    if (szs[tot] == 0)
            ...
            ll j = 0;
            ll cnt = 1;
            for (auto pp = szs.rbegin(); pp != szs.rend(); pp++)
            {

                auto it = *pp;
                // deb(it);
                ll c = it.second;
                ans = ans + (pref[j + c] - pref[j]) * it.first;
                ll now = factorialMod(it.second, M);
                cnt = (cnt * now) % M;
                j += c;
            }
            cout << ans << " " << cnt << nn;
