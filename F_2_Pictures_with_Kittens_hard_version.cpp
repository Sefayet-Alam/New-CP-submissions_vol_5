...
int main()
{
    ...
    vector<vl>dp(5005,vl(5005,-M));
    //dp[i][j]->till position j, we took i elements
    dp[0][0]=0; //till position 0, we took 0 elements->ans=0;
    for(ll i=1;i<=x;i++){
        deque<ll>dq;
        dq.push_back(0);
        for(ll j=1;j<=n;j++){
            while(dq.size() && dq.front()<j-k) dq.pop_front();
            dp[i][j]=dp[i-1][dq.front()]+vec[j];
            while(dq.size() && dp[i-1][dq.back()]<dp[i-1][j]) dq.pop_back();
            dq.push_back(j);
        }
    }
    ll res=-M;
