...
int main()
{
    ...
    while (t--)
    {
        ...
        for (ll i = 0; i < n; i++)
        {
            ...
            if (vec[i] == 0)
            {
                auto nxt1 = upper_bound(all(pos[1]), i);
                if (nxt1 == pos[1].end())
                    continue;
                auto nxt2 = upper_bound(all(pos[2]), i);
                if (nxt2 == pos[2].end())
                    continue;
                maxm = max((*nxt1), (*nxt2));
            }
            ...
            {
                auto nxt0 = upper_bound(all(pos[0]), i);
                if (nxt0 == pos[0].end())
                    continue;
                auto nxt2 = upper_bound(all(pos[2]), i);
                if (nxt2 == pos[2].end())
            ...
            auto er = upper_bound(all(pos[3]), i);
            if (er != pos[3].end())
            {
                if ((*er) < maxm)
                    ok = 0;
        ...
        for (ll i = 0; i < n; i++)
        {
            ...
            if (vec[i] == 0)
            {
                ...
                if (nxt1 == pos[1].end())
                    continue;
                auto nnxt1 = upper_bound(all(pos[1]), (*nxt1));
                if (nnxt1 == pos[1].end())
                    continue;
                maxm = (*nnxt1);
            }
