...
void minCostMaxFlow(ll _src, ll _dest)
{
    src = _src;
    dest = _dest;
    pll result = mp(0,0);
    ...
    // Run min-cost max-flow to push k flows
    while(spfa() && cnt < k)
    {
...
int main()
{
    ...
    addEdge(src, 1, k, 0);
    addEdge(n + n, dest, k, 0);
 
 
    minCostMaxFlow(src, dest);
