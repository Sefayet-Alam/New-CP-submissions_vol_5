...
ll func(ll i,ll f){
    ...
    if(i==0){
        ...
        if(f){
            if(vec[i]>=rev[i-1]) ret=min(ret,func(i+1,0));
            if(rev[i]>=rev[i-1]) ret=min(ret,c[i]+func(i+1,1));
        }
        else{
            if(vec[i]>=vec[i-1]) ret=min(ret,func(i+1,0));
            if(rev[i]>=vec[i-1]) ret=min(ret,c[i]+func(i+1,1));
        }
    }
    return dp[i][f]=ret;
}
