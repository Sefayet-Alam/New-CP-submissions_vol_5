...
int main()
{
    ...
    for (int i = 0; i < n; i++)
        cin >> p[i];
    sort(p, p + n, greater<long double>());

    zero = 1; //peviously did nt take's probability
    one = 0; //maximum probability
    for (ll i = 0; i < n && zero > one; i++)
    {
        one = (1.00 - p[i]) * one + p[i] * zero;
        zero = (1.00 - p[i]) * zero;
    }

    return cout << fixed << setprecision(10) << one << endl, 0;
}
