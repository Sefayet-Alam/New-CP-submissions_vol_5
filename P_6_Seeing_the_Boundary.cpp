...
/*
    Problem: Fence Post Visibility

    Farmer Don owns a W x H fenced field with integer-coordinate fence posts along its perimeter.
    He stands at a given position inside the field and wants to determine how many fence posts he can see.

    There are R rocks in the field, each represented as a convex polygon with integer-coordinate vertices.
    A fence post is visible if the line segment from Don to the post is not blocked by any rock.
    Additionally, if a rock vertex perfectly aligns with a fence post along Don’s line of sight, that post is blocked.

    Input:
    - W, H: Width and height of the field.
    - (x, y): Farmer Don's position.
    - R: Number of rocks.
    - For each rock:
      - V: Number of vertices.
      - V lines of (xi, yi): Coordinates of the rock's vertices in counterclockwise order.

    Output:
    - The number of visible fence posts.
...
Event get_next_post(Event curr, int n)
{
    ...
    {
        if (curr.loc.y)
            return {0, 0, {n, curr.loc.y - 1}};
        return {0, 0, {n - 1, 0}};
    }
    else if (!curr.loc.x)
    {
        if (curr.loc.y != n)
            return {0, 0, {0, curr.loc.y + 1}};
        return {0, 0, {1, n}};
    }
    else if (curr.loc.y == n)
    {
        if (curr.loc.x != n)
            return {0, 0, {curr.loc.x + 1, n}};
        return {0, 0, {n, n - 1}};
    }
    else
    {
        if (curr.loc.x)
            return {0, 0, {curr.loc.x - 1, 0}};
        return {0, 0, {0, 1}};
    }
}
