...
int main()
{
    ...
    while (t--)
    {
        ...
        while (z.size() < n )
        {
            ...
            ll last=-1;
            vector<pll>tmp;
            while (r<n && pos[p[r]-'a'].upper_bound(last)!=pos[p[r]-'a'].end())
            {
                z+=p[r];
                ll now=p[r]-'a';
                ll cur=*pos[now].upper_bound(last);
                pos[now].erase(cur);
                tmp.push_back({now,cur});
                r++;
                last=cur;
            }
            for(auto it:tmp){
                pos[it.first].insert(it.second);
            }
        }
        // deb(z);
        cout<<ans<<nn;
