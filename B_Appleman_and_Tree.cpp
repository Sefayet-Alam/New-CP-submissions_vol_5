...
void dfs(ll v)
{
    ...
    for (auto u : g[v])
    {
        ...
        dp[v][1] = ((dp[v][1] * (dp[u][0] + dp[u][1])) % M) % M;

        dp[v][1] = (dp[v][1] + dp[v][0] * dp[u][1] % M) % M;

        dp[v][0] = ((dp[v][0] * (dp[u][0] + dp[u][1])) % M) % M;
    }
    if (col[v])
    {
        dp[v][1] = dp[v][0]; // can't take any more black
        dp[v][0] = 0;        // can't happen like # of b vertices is 0
    }
}
int main()
{
