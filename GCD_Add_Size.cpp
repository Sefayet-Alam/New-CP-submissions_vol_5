...
ll func(ll i, ll sz)
{
    ...
    if (dp[i][sz] != -1)
        return dp[i][sz];
    ll ret = 0;
    ret = max(ret, func(i + 1, sz));
    ret = max(ret, GCD(vec[i], func(i + 1, sz - 1)));
    return dp[i][sz] = ret;
}
int main()
{
    ...
    while (t--)
    {
        ...
        for (ll i = 1; i < min(n, 500LL); i++)
        {
            ans = max(ans, i + func(0, i));
        }
        map<ll, ll> freq;
        for (ll i = 0; i < n; i++)
        {
        ...
        for (ll i = 0; i < n; i++)
        {
            ans = max(ans, vec[i] + freq[vec[i]]);
        }
        ll gc=0;
        for(ll i=0;i<n;i++) gc=GCD(vec[i],gc);
        ans=max(ans,gc+n);
        cout << ans << nn;
    }
