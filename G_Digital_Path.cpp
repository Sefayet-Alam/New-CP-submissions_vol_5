...
ll dp[N][N][5];
ll dfs(pll vertex, ll len)
{
    ll ans = 0;
    vis[vertex.first][vertex.second] = true;
    bool f = 0;
    if(dp[vertex.first][vertex.second][len]!=-1) return dp[vertex.first][vertex.second][len];
    // deb(vertex);
    ll now=g[vertex.first][vertex.second];
    for (pll &child : Move)
    {
    ...
    return dp[vertex.first][vertex.second][len] = ans;
}
int main()
{
    fast;
