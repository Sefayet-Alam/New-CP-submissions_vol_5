...
{
    ll a = 0, b = 0;
    for (ll i = 0; i < cyc.size(); i += 2)
        a += cyc[i];
    for (ll i = 1; i < cyc.size(); i += 2)
        b += cyc[i];
    return max(a - b, b - a);
}
ll odfunc(vl cyc)
        ...
        for (ll i = n; i >= 1; i--)
        {
            ...
            if (cyc.size() % 2 == 0)
            {
                ...
                if (cycval >= 0)
                    ...
                    rem.push_back(cycval);
            }
            else
            {
                cycval = odfunc(cyc);
                cycss.push_back(cycval);
            }
        }
        // for(auto it:cycss) deb(it);
        // deb(rem);
        sort(all(cycss), greater<ll>());
        for (ll i = 0; i < cycss.size(); i++)
        {
            if (i % 2 == 0)
            {
                ans += cycss[i];
