...
ll func(vl& gifts,ll tot, ll id)
{
    ...
    for (ll i = 0; i < n; i++)
    {
        ...
        {
            gifts[i] = 1;
            ans = max(ans, gift[i][id][0] + func(gifts, tot + cost[i][0], id + 1));
            gifts[i] = 0;
        }
        else if (gifts[i] == 1)
        {
            gifts[i] = 2;
            ans = max(ans, gift[i][id][1] + func(gifts, tot + cost[i][1], id + 1));
            gifts[i] = 1;
        }
    }
    return dp[mask] = ans;
