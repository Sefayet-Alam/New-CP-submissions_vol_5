    ...
    while (t--)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            if (i % 3 == 1)
            {
                if (i + 3 < n)
                {
                    f[i + 3] = max(f[i + 3], f[i] + Vote(s[0][i + 1], s[0][i + 2], s[0][i + 3]) + Vote(s[1][i], s[1][i + 1], s[1][i + 2]));
                    g[i + 3] = max(g[i + 3], g[i] + Vote(s[0][i], s[0][i + 1], s[0][i + 2]) + Vote(s[1][i + 1], s[1][i + 2], s[1][i + 3]));
                }
                f[i + 2] = max(f[i + 2], f[i] + Vote(s[1][i], s[0][i + 1], s[1][i + 1]));
                f[i + 2] = max(f[i + 2], g[i] + Vote(s[0][i], s[0][i + 1], s[1][i + 1]));
            }
        }
        cout << f[n] << nn;
    }
