...
bool cmp3(pair<pll, ll> a, pair<pll, ll> b)
{
    return a.first.second > b.first.second;
}
int main()
{
    ...
    while (t--)
    {
        ...
        sort(all(vec), cmp);
        ll hf=n/2;
        vector<pair<pll,ll>>l(vec.begin(),vec.begin()+hf);
        vector<pair<pll,ll>>r(vec.begin()+hf,vec.end());
        sort(all(l),cmp2);
        sort(all(r),cmp3);
        for(ll i=0;i<hf;i++){
            cout<<l[i].second<<" "<<r[i].second<<nn;
        }
    }
