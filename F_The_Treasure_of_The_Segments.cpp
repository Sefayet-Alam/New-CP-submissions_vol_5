...
int main()
{
    ...
    while (t--)
    {
      ...
      vector<ll>ms;
      for(ll i=0;i<n;i++){
        ms.push_back(vec[i].first);
      }
      sort(all(ms));
      ordered_multiset<ll>ms2;
      ll ans=n-1;
      for(ll i=0;i<n;i++){
        ll prev=0;
        if(ms2.size()){
            auto pos=ms2.order_of_key(vec[i].first);
            prev=pos;
        }
        ll af=0;
        if(ms.size()){
            ll pos=n-(upper_bound(all(ms),vec[i].second)-ms.begin());
            af=pos;
        }
        // deb(vec[i]);
        // deb2(prev,af);
        ans=min(ans,(af+prev));
        ms2.insert(vec[i].second);
      }
      cout<<ans<<nn;
    }
