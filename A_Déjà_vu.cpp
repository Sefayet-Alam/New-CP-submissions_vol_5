...
int main()
{
    ...
    ll n, q;
    cin >> n >> q;
    vector<ll>  plscrs(n, 0);
    for (ll i = 0; i < n; i++)
    {
        plscrs[i] = (n - 1 - i);
        vec.insert({-plscrs[i], i});
    }
    cout << plscrs << nn;
    ...
    while (q--)
    {
        ...
        x--;
        y--;
        auto it=vec.lower_bound({-plscrs[x],x});
        ll scr=-(*it).first;
        // deb(*it);
        auto need=vec.find_by_order(y);
        // deb(*need);
        ll nowneed=-(*need).first;
        ll add=nowneed-scr+q+1;

        cout<<add<<nn;
        plscrs[x]+=(add);
        vec.erase(it);
        vec.insert({-plscrs[x],x});
        // for(ll i=0;i<n;i++){
        //     deb2(i,plscrs[i]);
        // }
    }
