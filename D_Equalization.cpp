...
ll dp[62][62][62];
ll func(ll a,ll b,ll p,ll cost){
    if(a==b && a==0) return 0;
    if(p==0) return M;
    if(dp[a][b][p]!=-1) return dp[a][b][p];
    ll ret=M;
    ret=min(ret,func(a,b,p-1,cost/2));
    if(a>=p) ret=min(ret,cost+func(a-p,b,p-1,cost/2));
    if(b>=p) ret=min(ret,cost+func(a,b-p,p-1,cost/2));
    return dp[a][b][p]=ret;
}
