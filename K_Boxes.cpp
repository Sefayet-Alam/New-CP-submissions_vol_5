...
int main()
{
    ...
    while (t--)
    {
        ...
        for (ll i = 0; i < k; i++)
        {
            stt.insert(vec[i].first);
            tot += vec[i].second;
        }
        for (ll i = k; i < n; i++)
        {
            pqq.insert({-vec[i].second, vec[i].first});
        }
        sort(all(vec), cmp);
        unordered_map<ll, ll> answ;
        for (ll i = 0; i < n; i++)
        {
            // deb(stt);
            // deb(pqq);
            // deb(tot);
            if (stt.find(vec[i].first) != stt.end())
            {
                stt.erase(vec[i].first);
                tot -= vec[i].second;
                if (pqq.size())
                {
                    auto tp = *pqq.begin();
                    ...
                    ll now = (-tp.first);
                    tot += now;
                    stt.insert(tp.second);
                }
            }
            else
            {
                pqq.erase({-vec[i].second, vec[i].first});
            }
            answ[vec[i].first] = tot;
        }
        for (ll i = 0; i < n; i++)
        {
            cout << answ[a[i]] << " ";
