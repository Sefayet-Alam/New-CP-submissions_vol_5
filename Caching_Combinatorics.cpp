...
//sterling numbers: given an permutations a, how many ways are there
// to convert to b if in an operation , u can choose any element and
//move it to the first
ll saved[6006][6006];

ll sterling(ll n, ll k)
{
    if (k == n)
        return 1;
    if (k > n)
    ...
    ret = (sterling(n - 1, k - 1) % M + ((k * sterling(n - 1, k)) % M)) % M;
    return ret;
}
    ...
    while (t--)
    {
        ...
        for (ll i = n - 1, j = n - 1; i >= 0; i--)
            if (a[i] == b[j])
                j--, len++;
        ll ans = 0;
        for (ll i = n; i >= max(1ll, n - len); i--)
            ans = (ans + sterling(m,i)) % M;
