...
bool canFindX(ll num, ll mid)
{
    if (mid < 0)
        return false;
    ll remainder = mid;
    ll dividend = num - remainder;
    if (dividend <= 0)
        return false;
    return (dividend >= (remainder + 1));
}
        ...
        for (ll i = 0; i < n; i++)
        {
            for (ll j = i; j < n; j++)
            {
                ...
                for (ll k = 1; k < tmp.size(); k++)
                {
                    ...
                    {
                        ll aa = LLONG_MAX;
                        for (ll x = last + 1; x <= min(last + 1000, tmp[k]);x++)
                        {
                            ll a_prime = tmp[k] % x;
                            if (a_prime > last)
                            {
                    ...
                    // try to make tmp[k]>=x
                    // tmp[k]%=x where x is a positive number ..tmp[k]<=1e9
                }
                // deb(tmp);
                if (check(tmp) && ok)
                    ans++;
