...
int main()
{
    ...
    while (t--)
    {
        ...
        while (q--)
        {
            ...
            if (now >= tim)
            {
                ll k = histry[now - tim].first;
                if (k != -1)
                {
                    char ch = histry[now - tim].second.first + 'a';
                    hs.upd(k, ch);
                    char ch2 = histry[now - tim].second.second + 'a';
                    hs2.upd(k, ch2);
                }
            }
            if (op == 1)
            {
                ...
                hs.upd(x, '?');
                hs2.upd(x, '?');
                histry.push_back({x, {a[x - 1] - 'a', b[x - 1] - 'a'}});
            }
            else if (op == 2)
            {
                ll c1, pos1, c2, pos2;
                ...
                if (c1 == c2)
                {
                    if (c1 == 1)
                    {
                        hs.upd(pos1, a[pos2 - 1]);
                        hs.upd(pos2, a[pos1 - 1]);
                        swap(a[pos1 - 1], a[pos2 - 1]);
                    }
                    else
                    {
                        hs2.upd(pos1, b[pos2 - 1]);
                        hs2.upd(pos2, b[pos1 - 1]);
                ...
                else
                {
                    if (c1 == 1 && c2 == 2)
                    {
                        hs.upd(pos1,b[pos2-1]);
                        hs2.upd(pos2,a[pos1-1]);
                        swap(a[pos1 - 1], b[pos2 - 1]);
                    }
                    else
                    {
                        hs.upd(pos2,b[pos1-1]);
                        hs2.upd(pos1,a[pos2-1]);
                        swap(a[pos2 - 1], b[pos1 - 1]);
                    }
                }
                histry.push_back({-1, {'a', 'b'}});
            }
            else
            {
                if(hs.get_hash()==hs2.get_hash()) cout<<"YES"<<nn;
                else cout<<"NO"<<nn;
                histry.push_back({-1, {'a', 'b'}});
            }
            now++;
