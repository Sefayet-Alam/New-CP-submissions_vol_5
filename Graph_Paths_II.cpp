...
struct Matrix {
    ...
    // Constructor to initialize matrix with INF
    Matrix() {
        REP(i, 105) REP(j, 105) a[i][j] = INF;
    }

    // Matrix multiplication (Floyd-Warshall style min-sum product)
    Matrix operator*(const Matrix &other) const {
        Matrix product;
        REP(i, n) REP(j, n) {
            product.a[i][j] = INF; // Initialize with INF
            REP(k, n) {
...
Matrix expo_power(Matrix a, ll k) {
    Matrix product;
    REP(i, n) product.a[i][i] = 0; // Identity matrix for shortest paths
...
int main() {
    ...
    // Compute shortest path matrix after k steps
    Matrix p = expo_power(mat, k);
    
    // Get shortest path from node 0 to node (n-1)
    ll ans = p.a[0][n - 1];
    cout << (ans >= INF ? -1 : ans) << "\n";
