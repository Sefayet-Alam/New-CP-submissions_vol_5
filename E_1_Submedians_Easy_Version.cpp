...
vector<ll> a(N);
ll n, k;
tuple<bool, ll, ll> func(ll pos)
{
    vector<ll> pref(n + 1, 0);
    for (ll i = 0; i < n; ++i)
        pref[i + 1] = pref[i] + (a[i] >= pos ? 1 : -1);
    ...
    for (ll r = k; r <= n; r++)
    {
        ...
        if (minid[pr] != -1 && pref[r] - minm[pr] >= 0)
        {
            return {true, minid[pr] + 1, r};
        }

        ll popp = pr ^ 1;
        if (minid[popp] != -1 && pref[r] - minm[popp] >= 1)
        {
            return {true, minid[popp] + 1, r};
        }
    }
    // cout<<"NOT OK"<<nn;
    return {false, -1, -1};
