...
int main()
{
    ...
    while (t--)
    {
        ...
        vector<ll> vec(n);
        cin >> vec;
        vector<ll> prefmin(n, M);
        prefmin[0] = vec[0];
        for (ll i = 1; i < n; i++)
        {
            prefmin[i] = min(prefmin[i - 1], vec[i]);
        }
        vector<ll> suffmax(n, 0);
        suffmax[n - 1] = vec[n - 1];
        for (ll i = n - 2; i >= 0; i--)
        {
        ...
        vector<ll> ans(n, 0);
        if (vec[0] == prefmin[n - 1] || vec[0] == suffmax[0] || (suffmax[1] >= vec[0]))
            ans[0] = 1;
        if (vec[n - 1] == prefmin[n - 1] || vec[n - 1] == suffmax[0] || (prefmin[n - 2] <= vec[n - 1]))
            ans[n - 1] = 1;
        for (ll i = 1; i < n - 1; i++)
        {
            if (vec[i] == prefmin[n - 1] || vec[i] == suffmax[0])
                ans[i] = 1;
            ll a = prefmin[i - 1];
            ll b = suffmax[i];
            if (min(a, b) == vec[i] || max(a, b) == vec[i])
                ans[i] = 1;
            a = prefmin[i];
            b = suffmax[i + 1];
            if (min(a, b) == vec[i] || max(a, b) == vec[i])
