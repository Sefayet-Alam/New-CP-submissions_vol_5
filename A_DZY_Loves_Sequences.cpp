...
int main()
{
    ...
    while (t--)
    {
        ...
        for (ll i = 0; i < vec.size() - 1; i++)
        {
            ...
            if (b.first == b.second)
            {
                ans = max(a.second - a.first + 2, ans);
                if (i + 2 < vec.size())
                {
                    if (v[a.second] + 1 < v[vec[i + 2].first])
                    {
                        ans = max(ans, vec[i + 2].second - a.first + 1);
                    }
                }
            }
            else if (a.first == a.second)
            {
                ans = max(b.second - b.first + 2, ans);
            }
            else
            {
                if (v[a.second] + 1 < v[b.first + 1])
                {
                    ans = max(ans, b.second - a.first + 1);
                }
                else if (v[a.second - 1] + 1 < v[b.first])
                {
                    ans = max(ans, b.second - a.first + 1);
                }
            }
        ...
        for (auto it : vec)
        {
            if(it.first!=0 || it.second!=n-1) ans = max(ans, it.second - it.first + 2);
