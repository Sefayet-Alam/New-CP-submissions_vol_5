...
double func(double mid){
    double ret=0.000;
    for(ll i=0;i<n;i++){
        ret=max(ret,1.00*(vec[i].second)+abs(vec[i].first-mid));
    }
    return ret;
}
...
double ts(double l, double r){
    ...
    while (r-l>eps){
        ...
        if (f1<f2) r=mid2;     //change f1>f2 if needed minimum
        else l = mid1;  
    }
                 //return the maximum of func(x) in [l, r]
    if(func(l)<=func(r)) return l;
    else return r;
}
