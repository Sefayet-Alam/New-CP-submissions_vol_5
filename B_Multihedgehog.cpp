...
bool bfs(ll n, ll k)
{
    ...
    while (!q.empty())
    {
        ...
        for (ll child : g[curr_v])
        {
            ...
            {
                if (vis[child] && level[child] == level[curr_v] + 1)
                {
                    cnt[child]++;
                }
                else if(level[child]==level[curr_v]) return false;
            }
        }
    }
    map<ll, vl> mpp;
    ...
    for (ll i = 1; i <= n; i++)
    {
        mpp[level[i]].push_back(i);
        maxlvl = max(maxlvl, level[i]);
    }
    if (mpp[maxlvl].size() != 1 || maxlvl!=k)
        return false;
    for (ll j = maxlvl; j > 0; j--)
    {
        for (auto it : mpp[j])
        {
            if (cnt[it]<3) return false;
        }
    }
    return true;
}
