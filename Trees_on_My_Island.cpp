...
///Picks theorem: S=I+B/2-1 , s->AREA, I->inside (lattice points), B->Boundary 
...
// Function to calculate the Shoelace area
ll shoelace_area(const vector<pair<ll, ll>>& points) {
    int n = points.size();
    ll area = 0;
    for (int i = 0; i < n; i++) {
...
// Function to count boundary lattice points
ll boundary_points(const vector<pair<ll, ll>>& points) {
    int n = points.size();
    ll boundary = 0;
    for (int i = 0; i < n; i++) {
        ll x1 = points[i].first, y1 = points[i].second;
...
void solve() {
    ...
    ll area = shoelace_area(points);
    ll boundary = boundary_points(points);
