...
int main()
{
    ...
    while (t--)
    {
        ...
        for (ll i = 0; i < 100; i++)
        {
            ...
            ll difftot = accumulate(all(diff), 0LL);

            reverse(all(rev));
            vector<ll> revdiff;
            for (ll i = 1; i < rev.size(); i++)
            {
                revdiff.push_back(rev[i] - rev[i - 1]);
            }
            ll revdifftot = accumulate(all(revdiff), 0LL);
            ll maxm = max({difftot, revdifftot});
            ans=max({ans,maxm,tot});

            // deb2(revdifftot,difftot);
            // deb2(rev,diff);
            vec.clear();
            if (maxm == revdifftot)
            {
