...
{
    deque<pair<ll, pll>> pqq;
    pqq.push_front({0, src});
    level[src.first][src.second] = 0;
    while (pqq.size())
    {
        auto now = pqq.front().second;
        ll v_dist = pqq.front().first;
        ll x = now.first;
        ll y = now.second;
        pqq.pop_front();
        if (v_dist > level[now.first][now.second])
            continue;

        for (auto it : Move)
        {
            ll childx = x + it.first;
            ll childy = y + it.second;
            if (isvalid(childx, childy))
            {
                ll wt = 0;
                if (grid[x][y] != grid[childx][childy])
                    wt = 1;
                if (level[x][y] + wt < level[childx][childy])
                {
                    ...
                    if (wt)
                    {
                        pqq.push_back({level[childx][childy], {childx, childy}});
                    }
                    else
                    {
                        pqq.push_front({level[childx][childy], {childx, childy}});
                    }
                }
            }
        }
