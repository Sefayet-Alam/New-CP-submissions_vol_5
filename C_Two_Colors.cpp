...
int main()
{
    ...
    while (t--)
    {
        ...
        for(ll i=1;i<n;i++){
            ll fst=i;
            ll scnd=n-i;
            ll canf=m-(lower_bound(all(vec),fst)-vec.begin());
            ll cans=m-(lower_bound(all(vec),scnd)-vec.begin());
            ll maxm=max(fst,scnd);
            ll tomin=m-(lower_bound(all(vec),maxm)-vec.begin());
            ll now=max(0LL,canf*cans-tomin);
            // deb2(fst,scnd);
            // deb2(canf,cans);
            // deb2(now,tomin);
            ans+=now;
