...
// Finds the minimum number of right shifts (0-based indexing)
int minRightShiftsToOriginal(const string& s) {
    int n = s.size();
    string doubled = s + s;
    vector<int> Z = computeZ(doubled);
...
int main()
{
    ...
    while (t--)
    {
        ...
        for(ll i=1;i<=n;i++){
            ...
            cur=minRightShiftsToOriginal(p);
            // deb(cur);
            lc=LCM(lc,cur);
        }
        cout<<lc<<nn;
