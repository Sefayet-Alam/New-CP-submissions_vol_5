...
ll powerMod(ll x, ll y, ll p){
    if(stored.find({x,y})!=stored.end()) return stored[{x,y}];
    ll res = 1 % p;
    x = x % p;
    while (y > 0){
        if (y & 1) res = (res * x) % p;
...
ll func(ll p,ll rem){
    if(rem==0){
        return powerMod(now,p,M);
    }
    if(dp[p][rem]!=-1) return dp[p][rem];
    ll ret=0;
    ll x=inverseMod(p+1,M);
    for(ll i=0;i<=p;i++){
        ret=(ret+x*func(i,rem-1))%M;
    }
    return dp[p][rem]=ret;
}
