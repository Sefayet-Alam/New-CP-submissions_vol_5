...
node mergeNode(const node &l, const node &r) {
    ...
    // prefix length
    if (l.prelen == l.len && l.prech == r.prech)
        res.prelen = l.prelen + r.prelen;
    else
        res.prelen = l.prelen;

    // suffix length
    if (r.suflen == r.len && r.sufch == l.sufch)
        res.suflen = r.suflen + l.suflen;
    else
        res.suflen = r.suflen;
...
void build(int idx, int st, int en) {
    ...
    int mid = (st + en) >> 1;
    build(idx << 1, st, mid);
    build(idx << 1 | 1, mid + 1, en);
    seg[idx] = mergeNode(seg[idx << 1], seg[idx << 1 | 1]);
}

void upd(int idx, int st, int en, int pos, char c) {
    if (st == en) {
        seg[idx] = node(c);
        return;
    }
    int mid = (st + en) >> 1;
    if (pos <= mid)
        upd(idx << 1, st, mid, pos, c);
    else
        upd(idx << 1 | 1, mid + 1, en, pos, c);
    seg[idx] = mergeNode(seg[idx << 1], seg[idx << 1 | 1]);
}

node query(int idx, int st, int en, int l, int r) {
    if (r < st || l > en) return node();
    if (l <= st && en <= r) return seg[idx];
    int mid = (st + en) >> 1;
    node left = query(idx << 1, st, mid, l, r);
    node right = query(idx << 1 | 1, mid + 1, en, l, r);
