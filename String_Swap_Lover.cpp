...
struct segment_tree {
    ll size;
    vector<item> tree;
    item neutral = {0, 0, 0, false};

    item merge(item& a, item& b) {
        item res;
        res.c0 = a.c0 + b.c0;
        res.c1 = a.c1 + b.c1;
        res.inv = a.inv + b.inv + (a.c0 * b.c1);
        res.lazy = false;
        return res;
    }

    void add(ll x) {
        tree[x].inv = tree[x].c0 * tree[x].c1 - tree[x].inv;
        swap(tree[x].c0, tree[x].c1);
    ...
    void RangeAdd(ll l, ll r, ll x, ll lx, ll rx) {
        if (lx >= r || l >= rx) {
            return;
        }
        if (lx >= l && rx <= r) {
        ...
        push(x, lx, rx);
        ll m = (lx + rx) / 2;
        RangeAdd(l, r, 2 * x + 1, lx, m);
        RangeAdd(l, r, 2 * x + 2, m, rx);
        tree[x] = merge(tree[2 * x + 1], tree[2 * x + 2]);
    }

    void RangeAdd(ll l, ll r) {
        if (l < r) {
            RangeAdd(l, r, 0, 0, size);
        }
    }
