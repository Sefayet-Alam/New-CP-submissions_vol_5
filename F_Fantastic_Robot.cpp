...
pll calc(string r, ll nowx, ll nowy)
{
    for (auto it : r)
    {
        if (it == 'U')
        {
            if (isvalid(nowx, nowy - 1))
            {
                nowy--;
            }
        }
...
void dijkstra(ll srcx, ll srcy)
{
    ...
    while (pq.size())
    {
        ...
        for (auto it : ords)
        {
            auto [child_vx, child_vy] = calc(it, vx, vy);
            ll cost = abs(child_vx - vx) + abs(child_vy - vy);

            if (cost + level[child_vx][child_vy] < cost[child_vx][child_vy])
            {
