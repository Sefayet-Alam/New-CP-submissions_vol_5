...
ll dp[102][102][12][12];
ll func(ll rem,ll rem2,ll curk,ll curk2){
    if(rem==0 && rem2==0){
        return 1;
    }
    if(dp[rem][rem2][curk][curk2]!=-1) return dp[rem][rem2][curk][curk2];
    ll ret=0;
    if(curk+1<=k1 && rem){
        ret+=func(rem-1,rem2,curk+1,0);
        ret%=M;
    }
    if(curk2+1<=k2 && rem2){
        ret+=func(rem,rem2-1,0,curk2+1);
        ret%=M;
    }
    return dp[rem][rem2][curk][curk2]=ret%M;
}
int main()
{
    fast;
