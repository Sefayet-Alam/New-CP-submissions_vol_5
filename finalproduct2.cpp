...
ll nCrMod(ll n, ll r, ll x)
{
    ...
    for (ll i = 0; i < r; i++)
    {
        numerator = (numerator * ((n_mod - i + x) % x)) % x;
    }

    ll denominator = factorialMod(r, x);
    
    return (numerator * inverseMod(denominator, x)) % x;
}
...
ll ways(ll x)
{
    auto fact = getfacts(x);
    ll ret = 1;
    for (auto it : fact)
    {
        ret = (ret * nCrMod(n + it.second - 1, n - 1, M)) % M;
    }
    return ret;
}
...
void setIO()
{
    ...
    freopen("output_fproduct_ch2.txt", "w", stdout);
#endif // ONLINE_JUDGE
}
...
int main()
{
    ...
    while (t--)
    {
        ...
        cin >> n >> a >> b;
        ll ans = 0;
        for (ll i = 1; i * i <= b && i <= a; i++)
        {
            if (b % i)
                continue;
            ll fsthalf = ways(i);
            ll othr = b / i;
            ll scndhalf = ways(othr);
            ans = (ans + (fsthalf * scndhalf) % M) % M;
