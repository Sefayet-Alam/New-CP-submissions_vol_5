...
int main()
{
    ...
    pow2[0] = 1;
    for (ll i = 1; i < MAXN; i++) {
        invFM[i] = inverseMod(factorialMod(i,M),M);
        pow2[i] = (pow2[i - 1] * 2) % M;
    }
    for (ll r = 1; r < MAXR; r++)
    {
        vector<ll> prevS(MAXN, 1);
        for (ll c = 1; c < MAXN; c++)
        {
            vector<ll> currS(MAXN, 0);
            ll split_ways = (pow2[c] - 2+M)%M;
            for (ll n = 1; n < MAXN; n++)
            {
                ll dp_val = 0;
                if (n >= r)
                {
                    ll s_val = prevS[n - r];
                    dp_val = (factorialMod(n-1,M) * s_val) % M;
                }
                ll term = (dp_val * split_ways) % M;
                ans[r][n][c] = (ans[r][n][c - 1] + term) % M;
                ll term_S = (dp_val * invFM[n]) % M;
                currS[n] = (currS[n - 1] + term_S) % M;
            }
            prevS = currS;
        }
    }
