...
// Query for count of numbers < x in range [lx, rx]
...
// Point update: assign x to index i
...
void update_handler(int i, int x)
{
    update(1, 1, n, i, x);
}

ll query_handler(int lx, int rx, int x)
{
    return query(1, 1, n, lx, rx, x);
}
...
int main()
{
    ...
    while (t--)
    {
        ...
        for (ll i = 1; i < n; i++)
        {
            if (a[i] == -1)
                continue;
            ans += n - i - query_handler(i + 1, n, i + 1);
        }
        cout << ans << nn;
