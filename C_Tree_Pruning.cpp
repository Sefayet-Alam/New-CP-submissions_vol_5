...
void dfs(ll vertex, ll par = 0)
{
    ...
    subtree[vertex] = level[vertex];
    for (auto child : g[vertex])
    {
        if (child == par)
            continue;
        dfs(child, vertex);
        subtree[vertex] = max(subtree[vertex], subtree[child]);
    }
}
void reset(ll n)
{
...
int main()
{
    ...
    while (t--)
    {
        ...
        vector<ll>lens;
        for(ll i=1;i<=n;i++){
            lens.push_back(subtree[i]);
        }
        sort(all(lens));
        vector<ll>inlvl(n+2,0);
        for(ll i=1;i<=n;i++){
            inlvl[level[i]]++;
            maxlvl=max(maxlvl,level[i]);
        }
        ll ans=n-1;
        ll now=n;
        ll ex=0;
        for(ll lvl=maxlvl;lvl>1;lvl--){
            auto pos=lower_bound(all(lens),lvl)-lens.begin();
            ll cur=pos+ex;
            ans=min(ans,cur);
            ex+=inlvl[lvl];
            now-=inlvl[lvl];
