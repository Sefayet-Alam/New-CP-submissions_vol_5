...
// if p is a substring of s
//  then refrain(p)=length(p)*(no of occurence of p in s)

pair<ll, string> findRefrain(string &s, const vector<ll> &lcp, const vector<ll> &suffix)
{
    s.pop_back();
    stack<ll> monoStack;
    const ll n = lcp.size();
...
int main()
{
    ...
    for(ll i=1;i<=n+2;i++) pref[i]+=pref[i-1];
    for(ll i=1;i<n;i++){
        cout<<pref[i]<<" ";
    }
    cout<<nn;
