...
int main()
{
    ...
    while (t--)
    {
        ...
        for (ll i = 0; i < n; i++)
        {
            if(i!=n-1) edges.push_back({vec[i+1]%vec[i],{vec[i],vec[i+1]}});
            for (ll j = 2*vec[i]; j <= maxm; j += vec[i])
            {
                ll lb=lower_bound(all(vec),j)-vec.begin();
                if(lb!=n){
                    edges.push_back({vec[lb]%vec[i],{vec[i],vec[lb]}});
                }
            }
        }
        ll ans=0;
