...
int main()
{
    ...
    vec.resize(n);
    cin>>vec;
    vector<ll>prior(n+1,0);
    for(ll i=0;i<n;i++){
        prior[vec[i]]=i+1;
    }
    set<pair<ll,tuple<ll,ll,ll>>> q;
    
    q.insert({0,{0,prior[1],1}});
    vis[1]=1;
    level[1]=0;
    vector<ll>tmp;
    while(!q.empty()){
        auto tp=*q.begin();
        auto [parpr,curpr,cur_v]=tp.second;
        tmp.push_back(cur_v);
        q.erase(q.begin());
        for(ll child:g[cur_v]){
            if(!vis[child]){
                ...
                q.insert({level[child],{prior[cur_v],prior[child],child}});
            }
        }
    }

    // deb(tmp);
    if(tmp==vec) cout<<"Yes"<<nn;
    else cout<<"No"<<nn;
